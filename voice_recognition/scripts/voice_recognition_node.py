...
class voice_recognition():
    def __init__(self):
        # Initialize publishers for status and command topics
        ...
        # Subscribe to "/zimmer/status" topic to receive messages triggering the change of interpreter
    ...
    def data_record(self):
        ...
        # Find signed integer numbers in the string
        numbers_only = re.findall(r'[-+]?\d+', data_str)
        for number in numbers_only:
            self.data_save += [number]
    
    def command_recognition(self):
        data_array = np.array(self.data_save, dtype=np.float64) # Convert data_save to a NumPy float64 array
    ...
    def send_command(self):
        msg = str(self.voice_command) + ' recognized with ' + str(round(self.confidence, 3)) + ' of confidence'
